...
function CalculateTotal() {
	...
	*/
	...
	/*  CONVERT: Sandwich code
		...
		If nothing is selected, set divErrors to "Select a sandwich", and exit the function.
		But how do you know if nothing was selected?  Use a boolean variable in the If statement, then check its value after 
		you get out of the loop. Remember: Your code should be flexible enough so that if the number of sandwiches change, your code 
		can still work.  Afterall, that's one of the reasons we're using a loop.
		
		Do NOT call the GetSandwichName() function.  Incorporate its code here, and remove it.
	*/
	for (i = 0; i <= radSandwich.length - 1; i++) {
		var bCheckRadioSand = false;
		
		if (radSandwich[i].checked === true) {
		 	g_sSandwich = radSandwich[i].value;
		 	bCheckRadioSand = true;
			
			break;
		} 
	}
	
	if (bCheckRadioSand === false) {
	...
	*/
	for (i = 0; i <= radSize.length - 1; i++) {
		var bCheckRadioSize = false; 

		if (radSize[i].checked === true) {
			bCheckRadioSize = true;
			
			sSizeString = radSize[i].title.substr(1,4);
			fSizeFloat = Number(sSizeString);
	
			g_fTotal += fSizeFloat;
			g_sSize = radSize[i].value;

			break;
		}
	}
	
	if (bCheckRadioSize === false) {
	...
	*/
	for (i = 0; i <= chkExtras.length - 1; i++) {
		
		if (chkExtras[i].checked === true) {
			g_sExtras += chkExtras[i].value + " ";
			console.log(chkExtras[i].value);
