...
function Init() {
	...
	var btnProcessOrder = document.getElementById("btnProcessOrder");
	...
	btnCalculateTotal.onclick = CalculateTotal;
	btnProcessOrder.onclick = ProcessOrder;


	// Version 2
	// DO IT: You need to hook up an event handler that runs whenever the user selects a different Payment option.  This is the "onchange" event.  I suggest you use an anonymous function, and make use of the *selectedIndex* property to see if they chose the credit card.  This function will check to see if the user selected the Credit card option.  If they did, set the CSS visibility property to "visible", otherwise set it to "hidden".
...
function CalculateTotal() {
	...
	/*  DO IT:
		...
		Within each IF statement remove the line of code you wrote for Version 1.  Replace it with a call to a function (that you will write) named GetSandwichName().  When you call this function, pass it one parameter - the index of the radSandwich radio button that the user checked.  More info on the function itself is given below.
	*/
	
	if(radSandwich[0].checked === true) {
		g_sSandwich = radSandwich[0];
	
	} else if(radSandwich[1].checked === true) {
		g_sSandwich = radSandwich[1];
	
	} else if(radSandwich[2].checked === true) {
		g_sSandwich = radSandwich[2];
	
	} else if(radSandwich[3].checked === true) {
		g_sSandwich = radSandwich[3];
	...
	*/
	if(radSize[0].checked === true) {
		g_fTotal += gc_fSandwichPrice;
		g_sSize = radSize[0].value;
		
	} else if(radSize[1].checked === true) {
		g_fTotal += gc_fSandwichPrice;
		g_sSize = radSize[1].value;
	
	} else if(radSize[2].checked === true) {
		g_fTotal += gc_fSandwichPrice;
		g_sSize = radSize[2].value;
	...
	// Version 2
	/*
		In this version, the sandwiches are not all the same price.  The price of each sandwich is contained within the title attribute of the radSandwich radio button for that sandwich (take a look at the html to verify this).
	...
	*/
	...
	/* DO IT:
		...
		Remove each IF statement you wrote for Version 1.  Replace it with a call to a function (that you will write) named GetExtraUpdateTotal().   When you call this function, pass it one parameter - the index of the chkExtras checkbox that the user checked.  More info on the function itself is given below.
	*/
	if(chkExtras[0].checked === true) {
		g_sExtras += " Deep-Fried Spam";
	...
	if(chkExtras[2].checked === true) {
		...
		if(chkExtras[1].checked === false && chkExtras[0].checked === false) {
			g_sExtras += " Secret Sauce";
		
		} else {
			g_sExtras += ", and Secret Sauce";
		}
	}
...
/* DO IT:
	...
	This function takes one parameter, named p_iSandwichIndex, which is a radSandwich radio button index, i.e. the index of the Sandwich they selected.
...
/* DO IT:
	...
	This function takes one parameter, named p_iSizeIndex, which is a radSize radio button index, i.e. the index of the radSize radio button that they selected.
	The function should assign the *value* of the selected size to a global var name g_sSize.
	Also, it must update the Total with the price for the size they selected.  The price is located in the title attribute of the radio button (take a look).
...
function ProcessOrder() {
	...
	*/
	...
	divOrder.innerHTML += "<p>You have ordered a " + g_sSize + " " + g_sSandwich.value + " with " + g_sExtras + ".</p>";
